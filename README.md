...
- **`/batch-apply`** turns a strong ranked shortlist into individually tailored, ATS-checked CV packets and a review queue. Every CV uses the same verified candidate foundation but is freshly tailored to that job's actual description. It never submits applications or accepts portal terms: you approve each packet and control every final submission.
...
│   │   ├── batch-apply.md              # /batch-apply creates individualized review-ready CV packets
