...
- If `$ARGUMENTS` looks like a URL, use `WebFetch` to retrieve the job posting content. If it returns empty (common on client-side-rendered ATS pages — Workday, Ashby, some Greenhouse-embedded listings) and `FIRECRAWL_API_KEY` is set (see `.agents/skills/firecrawl-search/SKILL.md`), retry with `firecrawl-search detail <url>` before concluding the posting is unreachable.
