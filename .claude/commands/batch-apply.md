...
1. Fetch the URL. If it returns empty and `FIRECRAWL_API_KEY` is set (see `.agents/skills/firecrawl-search/SKILL.md`), retry with `firecrawl-search detail <url>` - client-side-rendered ATS pages (Workday, Ashby, some Greenhouse-embedded listings) are frequently just a JS-rendering gap, not a dead posting. If it is closed, dead, redirects to a generic list, or still lacks a usable description after that retry, set its `status` to `expired` in `seen_jobs.json`, record the reason, and skip it.
