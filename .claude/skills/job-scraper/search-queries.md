...
The `site:` query templates in this file are the **WebSearch fallback** — for portals without a CLI, company career pages, or when a CLI fails. If `firecrawl-search` is enabled (needs `FIRECRAWL_API_KEY`; see `.agents/skills/firecrawl-search/SKILL.md`), prefer `firecrawl-search search --domain naukri.com` / `--domain foundit.in` over the bare WebSearch `site:` fallback below for those two — it returns structured, individually-fetchable results instead of a snippet list.
